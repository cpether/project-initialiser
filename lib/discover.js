...
function extractEnvVars(server) {
  ...
  const scan = (s) => {
    ...
    for (const m of s.matchAll(pattern)) {
      const name = m[1] || m[2];
      if (name === 'CLAUDE_PLUGIN_ROOT') continue;
      vars.add(name);
    }
  };
...
function discoverUserMcps() {
  const data = jsonFile.readOptional(path.join(os.homedir(), '.claude.json'));
...
function readPluginMcpServers(installPath) {
  const merged = {};
  const mcpJson = readJsonSafe(path.join(installPath, '.mcp.json'));
  if (mcpJson && mcpJson.mcpServers) Object.assign(merged, mcpJson.mcpServers);
  const pluginJson = readJsonSafe(path.join(installPath, '.claude-plugin', 'plugin.json'));
  if (pluginJson && pluginJson.mcpServers) {
...
function discoverPluginMcps() {
  const out = [];
  for (const inst of discoverInstalledPlugins()) {
    const servers = readPluginMcpServers(inst.installPath);
...
function discoverProjectMcps(repoRoot) {
  ...
  const data = jsonFile.readOptional(p);
  if (!data || !data.mcpServers) return [];
  return Object.entries(data.mcpServers).map(([name, cfg]) => {
...
function discoverLocalMcps(repoRoot) {
  if (!repoRoot) return [];
  const data = jsonFile.readOptional(path.join(os.homedir(), '.claude.json'));
  if (!data || !data.projects) return [];
  const project = data.projects[repoRoot];
  if (!project || !project.mcpServers) return [];
  return Object.entries(project.mcpServers).map(([name, cfg]) => {
    const wrapped = isClaudeInitWrapped(cfg);
    return {
      ...normalizeServer(name, cfg, 'local'),
      claudeInitWrapped: wrapped,
    };
  });
}
