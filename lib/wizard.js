...
async function applyLocalMigrations(root, plan, projData) {
  ...
  for (const name of plan.toMigrate) {
    ...
    for (const envKey of candidate.plan.secrets) {
      ...
      if (!result) {
        console.log(dim(`  skipped local ${name}; ${envKey} was not configured.`));
        mappings.length = 0;
        break;
      }
      mappings.push({ envKey, secretName: (result && result.storedAs) || envKey });
    }
    if (mappings.length !== candidate.plan.secrets.length) continue;
...
async function applyProjectMigrations(root, projectMcps, plan, projData) {
  ...
  for (const name of plan.toMigrate) {
    ...
    for (const envKey of candidate.plan.secrets) {
      const result = await configureSecret(envKey, root, projData);
      if (!result) {
        console.log(dim(`  skipped ${name}; ${envKey} was not configured.`));
        mappings.length = 0;
        break;
      }
      mappings.push({ envKey, secretName: (result && result.storedAs) || envKey });
...
async function runInit({ root, isGit }) {
  ...
  const localMigrated = await applyLocalMigrations(root, localPlan, projData);
