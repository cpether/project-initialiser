...
async function pickLocalMigrations(localMcps, root) {
  if (localMcps.length === 0) return { toMigrate: [], migrateCandidates: [], skipped: [] };
  const allWithPlans = localMcps.map((m) => {
    const cfg = scopeMove.entryAtScope(root, 'local', m.name);
    return { name: m.name, cfg, plan: migrate.planMigration(cfg || {}) };
  });
  const migrateCandidates = allWithPlans.filter((x) => x.plan.canMigrate);
  const skipped = allWithPlans.filter((x) =>
  ...
  if (migrateCandidates.length > 0) {
    console.log('');
    toMigrate = await picker.checkbox({
      message: `Migrate local entries to claude-init exec? (removes \${VAR} env coupling)`,
      hint: '↑/↓ move · space toggle · enter confirm · q skip · default: none',
      choices: migrateCandidates.map((x) => ({
        name: x.name,
        value: x.name,
  ...
  if (skipped.length > 0) {
    console.log(dim('\n  local entries not auto-migrateable:'));
    for (const x of skipped) console.log(dim(`    ${x.name}: ${x.plan.reason}`));
  }
  return { toMigrate, migrateCandidates, skipped };
}

async function applyLocalMigrations(root, plan, projData) {
  ...
  for (const name of plan.toMigrate) {
    ...
    console.log(yellow(`\nMigrating local ${name} — configuring secrets…`));
    const mappings = [];
    for (const envKey of candidate.plan.secrets) {
      const result = await configureSecret(envKey, root, projData);
      mappings.push({ envKey, secretName: (result && result.storedAs) || envKey });
    }
    const newCfg = migrate.rewrap(candidate.cfg, candidate.plan, { execCommand: selfPath(), mappings });
    scopeMove.writeToScope(root, 'local', name, {
    ...
    console.log(green(`  migrated local ${name}`));
  }
}
...
async function applyProjectMigrations(root, projectMcps, plan, projData) {
  ...
  for (const name of plan.toMigrate) {
    ...
    const newCfg = migrate.rewrap(candidate.cfg, candidate.plan, { execCommand: 'claude-init', mappings });
...
async function runInit({ root, isGit }) {
  ...
  if (localMcps.length > 0) {
    ...
    localPlan = await pickLocalMigrations(localMcps, root);
  }
  ...
  await applyLocalMigrations(root, localPlan, projData);
  ...
  // 11. claude.ai + local — both write to disabledMcpServers (different name slices)
  if (claudeaiMcps.length > 0) {
  ...
  if (projectPlan.toMigrate.length) console.log(`  ${green('project migrated to claude-init:')} ${projectPlan.toMigrate.join(', ')}`);
  if (localPlan.toMigrate.length) console.log(`  ${green('local migrated to claude-init:')} ${localPlan.toMigrate.join(', ')}`);
